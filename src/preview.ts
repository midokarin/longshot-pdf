...
export class PaginationPreviewModal extends Modal {
	...
	private render(): void {
		...
		contentEl.empty();
		contentEl.createEl("h2", { text: "分页预览" });
		contentEl.createEl("p", {
			cls: "longshot-modal-note",
			text:
				"每页缩略图按真实版心绘制。断点不合适时用「↑ / ↓」把它挪到相邻的块边界；" +
				"也可以在正文里单独写一行 /// 来强制分页。",
		});
		...
		const reset = footer.createEl("button", { text: "恢复自动分页" });
		...
		const confirm = footer.createEl("button", { text: "按此分页导出", cls: "mod-cta" });
	...
	private createCard(parent: HTMLElement, index: number): PageCard {
		...
		const up = controls.createEl("button", { text: "↑ 断点上移" });
		up.addEventListener("click", () => this.nudge(index, "up"));
		const down = controls.createEl("button", { text: "↓ 断点下移" });
	...
	private refreshCard(index: number, card: PageCard): void {
		...
		try {
			...
			card.thumbWrap.createDiv({ cls: "longshot-preview-failed", text: "缩略图渲染失败" });
		}
		const ratio = (slice.end - slice.start) / Math.max(1, this.ctx.capacity);
		card.label.setText(`第 ${index + 1} 页 · 填充 ${Math.round(ratio * 100)}%`);
	...
	private refreshSummary(): void {
		this.summaryEl?.setText(`共 ${this.slices.length} 页，纸张 ${this.ctx.spec.geometry.paperWidthMm} × ${this.ctx.spec.geometry.paperHeightMm} mm`);
	}
